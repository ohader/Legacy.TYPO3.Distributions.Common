...
function get_additional_modify_hash()
{
	...
	if (file_exists($my_web_dir."/typo3conf/localconf.php")) {
		include_once($my_web_dir."/typo3conf/localconf.php");
		$parameters['@@ENCRYPTION_KEY@@'] =  $TYPO3_CONF_VARS['SYS']['encryptionKey'];
	} 
	if (!$parameters['@@ENCRYPTION_KEY@@'] || strlen($parameters['@@ENCRYPTION_KEY@@']) <= 0) {
		$parameters['@@ENCRYPTION_KEY@@'] = generate_password(64);
	}
    return $parameters;
}
